...
class WT_TicketImap{
	...
	public function plugins_loaded(){
		...
		add_action( 'wt/check_email_cron', array($this, 'check_emails'));
     ...
     * @return void
     */
    public function schedule_cron($rate = 'wt_5_min_cron') {
        if ( !wp_next_scheduled( 'wt/check_email_cron' ) ) {
            wp_schedule_event( time(), $rate, 'wt/check_email_cron');
        }
    }
    ...
    public function unshedule_cron(){
        wp_clear_scheduled_hook('wt/check_email_cron');
    }
	...
	public function check_emails(){
		...
		foreach($this->messages as $msg){
			...
			if($ticket){
				...
				$create = $wptickets->tickets->insert_comment($ticket, $body, $user_id);
				...
				// add new ticket
				$create = $wptickets->tickets->insert_ticket($msg['subject'], $body, $user_id);
	...
	private function is_support_user($email = false){
		...
		// skip to check if email is working
		return true;	
...
add_action('wptickets_loaded', 'wt_load_imap_addon');
function wt_load_imap_addon(){
	new WT_TicketImap();
}